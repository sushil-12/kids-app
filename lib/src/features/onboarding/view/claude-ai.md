...
2. **In-app service + feature-flag layer (2D)** — foundation for everything.
3. **W2: on-device adaptive difficulty** — high impact, free, no backend.
...
8. **W4 + W2 speech/scribble** polish.
...
- [ ] Lock decisions in §2A (BaaS, region, ML framework, model tier, TTS, difficulty).
- [ ] Start compliance review (§2C), especially India DPDP children's provisions.
- [ ] Add `core/services/` (API client) + Remote Config feature flags to the app.
- [ ] Spec the content-generation pipeline (W1) and the AI proxy (§4) in detail.
- [ ] Set model-API billing alerts + hard cap.
- [ ] Fold this plan's status into `CLAUDE.md` so Claude Code builds toward it.S